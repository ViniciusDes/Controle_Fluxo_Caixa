...
begin
    UnimSelect1.Visible := true;
    UnimSelect2.Visible := true;
    AtivaDataSet;
...
begin
   valor1 := StrToCurr(DbeditValor.Text);
   DbeditValor.Text := FormatFloat('#,.##', valor1);
   ...
   DataSetCaixaMov.FieldByName('SALDO_ANTERIOR').AsFloat := SomarSaldoAnterior(idConta1);  //APENAS VER O VALOR DA CONTA 1
   ValorTotalConta1 := DebitarSaldo; //DEBITANDO O VALOR DA PRIMEIRA CONTA, PASSANDO PRA VARIAVEL
  ...
  dData := date();
  UnimDBDatePicker1.Date := dData;
  DataSetCaixaMov.FieldByName('SALDO_ANTERIOR').AsFloat := SomarSaldoAnterior(idConta2); //APENAS VER O VALOR DA CONTA 2
  ...
  begin
   ...
   ShowMessage('Transferência concluida!');
...
procedure TFrmLancamentos.UnimFormCreate(Sender: TObject);
begin
  QueryCaixaMov.SQLConnection := MainmFormLogin.conexao;
  QueryFunc.SQLConnection := MainmFormLogin.conexao;
  QueryConfig.SQLConnection := MainmFormLogin.conexao;
...
begin
  ...
  DataSetCaixaMov.close;
  ClientDatasetFunc.close;
  DataSetConfig.close;
end;
